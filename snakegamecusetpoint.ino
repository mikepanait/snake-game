...
void setPoint() {   // Add parentheses for the function declaration
    // Fixed the for-loop syntax
    for (int i = 0; i < 32; i++) {  //acilisea este colon
        for (int j = 1; j < 32; j++) {   // acilisea este rows
            // You can perform some action here in the loops if needed
            matrix.setPoint(i,j,0xFF); // This defines the array inside the loop, but it should be outside if you want to use it outside the loops
        }
    }
}
void setRow(int row) {
